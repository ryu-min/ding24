...
import {GameSessionHistory, GameSessionListeners, GameSessionStorage, GameSessionTimer,} from './types';

const initialTimer: GameSessionTimer = {
  white: 600,
  black: 600
};
...
class GameSession {
  ...
  constructor(loadFromStorage?: boolean) {
    ...
    const session: GameSessionStorage = storedSessionValue && loadFromStorage ?
    ...
    this.history = session ? session.history : [
      {
        timer: {...initialTimer},
  ...
  getLoser() {
    return this.chess.in_checkmate() || this.timer.black === 0 ||
            this.timer.white === 0 ?
